...
//		queue.enqueueWriteBuffer(dev_convolution_mask, CL_TRUE, 0, convolution_mask.size()*sizeof(float), &convolution_mask[0]);
		...
		cl::Kernel kernel = cl::Kernel(program, "invert");// chaned from "identity" for tutorial 2 change this to decide what kernel to run or add another to run another kernel
		kernel.setArg(0, dev_image_input);// original
		kernel.setArg(1, dev_image_output);// picture with operation
//		kernel.setArg(2, dev_convolution_mask);
		...
		cerr << kernel.getWorkGroupInfo<CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE>(device) << ": Is the smallest work group size suggested" << endl; // get info
		cerr << kernel .getWorkGroupInfo<CL_KERNEL_WORK_GROUP_SIZE>(device) << ": Is the multiples up to the maximum work group size" << endl;
